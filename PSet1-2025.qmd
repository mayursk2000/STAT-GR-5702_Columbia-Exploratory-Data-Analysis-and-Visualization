...
all_counts$prop_dog <- all_counts$dog_count / all_counts$total

ggplot(all_counts, aes(x=dog_count, y=cat_count)) +
...
all_counts$type <- with(all_counts, 
...
top20 <- head(all_counts[order(-all_counts$total), ], 20)$name
all_counts$label <- ifelse(all_counts$name %in% top20, all_counts$name, "")

ggplot(all_counts, aes(x=dog_count, y=cat_count, color=type)) +
